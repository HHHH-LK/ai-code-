...
public class ComplexJsonStreamHandler implements Handler {
    ...
    private String handleJsonMessageChunk(String chunk, StringBuilder chatHistoryStringBuilder, Set<String> seenToolIds) {
        ...
        ThrowUtils.throwIf(typeEnum == null, ErrorCode.SYSTEM_ERROR, "不支持的流消息类型: {}");
        switch (typeEnum) {
            case AI_RESPONSE -> {
            ...
            case TOOL_REQUEST -> {
                ...
                if (toolId != null && !seenToolIds.contains(toolId)) {
                    ...
                    log.info("调用工具: {} 写入文件", toolRequestMessage.getName());
                    return "\n\n[选择工具]" + toolRequestMessage.getName() + " 写入文件\n\n";
                } else {
                    // 不是第一次调用这个工具，直接返回空
                    return "";
                }
            ...
            case TOOL_EXECUTED -> {
                ...
                log.info("工具{}执行完毕，写入文件{}，结果为{}", toolExecutedMessage.getName(), relativeFilePath, output);
                return output;
            }
            default -> {
